...
export class FacebookMarketplace extends BaseMarketplace {
  ...
  private async resolveLocation(query: string): Promise<LocationCoordinates | null> {
    ...
    try {
      ...
      const node = this.pickBestLocationNode(edges, cacheKey);
      const name =
  ...
  private pickBestLocationNode(edges: any[], query: string): any {
    const nodes = edges.map(edge => edge?.node).filter(Boolean);
    const normalizedQuery = query.toLowerCase();
    const queryWantsUs = /\b(united states|usa|u\.s\.|america|alabama|alaska|arizona|arkansas|california|colorado|connecticut|delaware|florida|georgia|hawaii|idaho|illinois|indiana|iowa|kansas|kentucky|louisiana|maine|maryland|massachusetts|michigan|minnesota|mississippi|missouri|montana|nebraska|nevada|new hampshire|new jersey|new mexico|new york|north carolina|north dakota|ohio|oklahoma|oregon|pennsylvania|rhode island|south carolina|south dakota|tennessee|texas|utah|vermont|virginia|washington|west virginia|wisconsin|wyoming)\b/.test(normalizedQuery);

    const exactCity = nodes.find(node => {
      const isCity = node.subtitle?.split(' \u00b7')[0] === 'City';
      const address = String(node.single_line_address || '');
      return isCity && this.isLikelyUsLocation(address);
    });

    if (queryWantsUs && exactCity) {
      return exactCity;
    }

    return exactCity ?? nodes[0];
  }

  private isLikelyUsLocation(address: string): boolean {
    return Array.from(US_STATE_NAMES).some(state => address.endsWith(`, ${state}`));
  }
